...
| 到达 / 在网 / 出账 | 存量状态 | 069 全业务资料表 | `is_cz`、`is_cancel_user`、`is_online_user` | 不是订单动作 |
...
| 销售品存量 / 在档 | 014 优惠资料表 `ads_yz_rpt_comm_cm_msdisc_final` | 020 销售品维表 | 041 优惠订单表 | 订单是动作，不是存量 |
...
| 全量科目级收入 | 048 全量科目级收入 | 指标 SQL 指定表 | 069 的收入标志字段 | 收入口径以指标或收入表为准 |
