...
| 群端 / 主从 AZ / A-B 端关系 | 用户附件种子表；附件只有群号/群端接入号时先 069 补 `serv_id` | 120 产品关联关系表、121 业务关联关系表；必要时回 069 补 Z 端 `acc_nbr`，再接 047/117/048 收入 | 仅用 033 `grp_acc_nbr` 反推 CRM A/Z 关系 | 输入群端或主端服务后，用 `a_prod_inst_id -> z_prod_inst_id` 找同组子端服务；两张关系表需 `city_id='200'`，合并后先去重；不要默认按 `z_prod_inst_id` 排序就是业务 A/B 端，需输出明细或确认排序口径 |
