...
| 群端 / 主从 AZ 关系补子端服务 | 附件群号、群端接入号、主端 `serv_id` | 需求给群端或主端，要找同组 A/B 端、Z 端、子端服务，再查号码、状态、收入或拆机 | 120 产品关联关系表 `dws_crm_cust.dws_prod_inst_rel_a`；121 业务关联关系表 `dws_crm_cust.dws_prod_inst_rel_grp_a`；069 全业务资料表 | 附件只有接入号时先用 069 补 `serv_id`；用 `serv_id = a_prod_inst_id` 分别查 120/121 的 `z_prod_inst_id`，限定 `city_id='200'`，`union all` 后按 `a_prod_inst_id,z_prod_inst_id` 去重；再用 `z_prod_inst_id = 069.serv_id` 补子端 `acc_nbr`，后续可接 047/117/048 收入 | 两张关系表可能同时命中同一对子，必须去重；一个主端可能多个 Z 端，默认输出明细，若要打成 A/B 两列需确认排序或端别口径，不能默认 `z_prod_inst_id` 小的是 A 端 |
