...
| 产品规格属性 / 特性 / attr_id | 产品规格特性值 | 105 特性资料表（**月表**取历史） | `serv_id` + `attr_id` + `attr_value1`；`par_corp_id='200'` | 历史快照用月表 `tb_pre_cm_attr_all_mon`；日表只在网 |
...
| 种子 serv_id + 拆机前一月产品规格属性（宽表） | 用户种子表（驱动）+ 069 **月表**（逻辑拆机月） | 105 特性资料**月表**（`attr_month_id=拆机月-1`）；字典（特性中文） | 特性日表（只在网）；069 日表（回溯场景） | 双账期 + 多 `attr_id` 宽表；详见 `verified-cases/VC-20260522-001` |
